...
<form action="http://localhost:8080/bank/user/login" method="post" id="formElm">
    <label> 用户名:
        <input type="text" name="username" id="usernameElm" value="请输入用户名"
               onclick="document.getElementById('usernameElm').value=''">
    </label>
    <label> 密码:
        <input type="text" name="password" id="passwordElm"
               value="请输入密码，要求8-20位，全部数字，不能有递增或者递减的密码。"
               onclick="document.getElementById('passwordElm').value=''">
    </label>
    <label> 选择ATM机:
    ...
    <script>
        ...
        const username_regex = /^[a-zA-Z]{3,6}$/;
        ...
        const password_regex = /^[a-zA-Z]{3,6}$/;
        ...
        submitObj.onclick = function () {
            ...
            let passwordElm = document.getElementById("passwordElm");
            // 创建一个flag用于判断表单正确性。
            let flag = false;
            // 开始判断表单。
            let isUsernameFlag = false;
            let isPasswordFlag = false;
